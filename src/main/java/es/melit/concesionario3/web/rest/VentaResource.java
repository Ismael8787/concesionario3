...
/**
 * REST controller for managing {@link es.melit.concesionario3.domain.Venta}.
 */
@RestController
@RequestMapping("/api")
public class VentaResource {
    ...
    public ResponseEntity<Venta> updateVenta(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Venta venta)
        ...
        Venta venta2 = v.get();
        this.cocheService.actualizarValorDespues(venta2);
        if (!ventaRepository.existsById(id)) {
            throw new BadRequestAlertException("Entity not found", ENTITY_NAME, "idnotfound");
        }
        this.cocheService.actualizarValorAntes(venta);
        Venta result = ventaService.save(venta);
        return ResponseEntity
            .ok()
    ...
    public ResponseEntity<Void> deleteVenta(@PathVariable Long id) {
        ...
        Optional<Venta> v = this.ventaRepository.findById(id);
        Venta venta2 = v.get();
        this.cocheService.eliminarValor(venta2);
